  ...
  return (
    <div className="w-full space-y-4 sm:space-y-5 md:space-y-6 py-6 sm:py-7 md:py-8 px-2">
      ...
      <div className="text-center">
        <div className="inline-flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 mb-4 sm:mb-5 md:mb-6 border-2 border-white/20 rounded-full">
          <Loader2 className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 text-white animate-spin" />
        </div>
        <h3 className="text-base sm:text-lg md:text-xl font-semibold text-white mb-1.5 sm:mb-2 px-4">
          {statusText[status]}
        </h3>
      ...
      <div className="space-y-2 sm:space-y-3">
        <div className="flex justify-between items-baseline">
          <span className="text-[10px] sm:text-xs uppercase tracking-wider text-white/40 font-semibold">
            Progress
          </span>
          <span className="text-xs sm:text-sm tabular-nums text-white/70 font-semibold">
            {Math.round(progress)}%
          </span>
        </div>
      ...
      <div className="text-center pt-3 sm:pt-4 px-4">
        ...
        {elapsedTime > estimatedTotal && (
          <p className="text-[10px] sm:text-xs text-white/50 font-medium mt-1.5 sm:mt-2">
