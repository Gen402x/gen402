/**
 * Token Price Service
 * Fetches live prices for $PAYPER token from various sources
 */
...
const priceOverrideRaw = process.env.NEXT_PUBLIC_PAYPER_PRICE_USD ?? process.env.PAYPER_PRICE_USD;
const PRICE_OVERRIDE = priceOverrideRaw ? parseFloat(priceOverrideRaw) : null;

const minPriceRaw = process.env.NEXT_PUBLIC_PAYPER_PRICE_MIN_USD ?? process.env.PAYPER_PRICE_MIN_USD;
const MIN_PRICE = minPriceRaw ? parseFloat(minPriceRaw) : null;

const fallbackPriceRaw = process.env.NEXT_PUBLIC_PAYPER_PRICE_FALLBACK_USD ?? process.env.PAYPER_PRICE_FALLBACK_USD;
...
/**
 * Fetches $PAYPER token price from DexScreener API
 */
  ...
  try {
    ...
    if (data.pairs && data.pairs.length > 0) {
      ...
      if (price && price > 0 && !isNaN(price)) {
        console.log(`💰 $PAYPER price from DexScreener: $${price}`);
...
/**
 * Fetches $PAYPER token price from Jupiter API (backup)
 */
  ...
  try {
    ...
    if (data.data && data.data[mintAddress]) {
      ...
      if (price && price > 0 && !isNaN(price)) {
        ...
        console.log(`💰 $PAYPER price from Jupiter: $${price}`);
        return price;
      }
    }
...
/**
 * Fetches the current $PAYPER token price in USD
...
export async function getTokenPriceUSD(): Promise<TokenPrice> {
  ...
  if (cachedPrice && Date.now() - cachedPrice.timestamp < CACHE_DURATION) {
    console.log('📦 Using cached $PAYPER price:', cachedPrice.priceUSD);
    return cachedPrice;
  }

  console.log('🔍 Fetching new $PAYPER price...');
...
// 10% buyback fee - used to buy back $PAYPER from the market
export const BUYBACK_FEE_PERCENTAGE = 10;

/**
 * Calculates how many $PAYPER tokens are needed for a given USD amount
 ...
 * - User pays: $0.03 / $0.0001 = 300 PAYPER
 * - 10% buyback: 30 PAYPER
 * - We receive: 270 PAYPER
 */
export async function calculateTokenAmount(usdAmount: number): Promise<{
  ...
  console.log(`💰 User pays: ${totalTokenAmount} $PAYPER`);
  console.log(`🔥 10% Buyback: ${feeAmount} $PAYPER`);
  console.log(`✅ We receive: ${baseTokenAmount} $PAYPER`);
