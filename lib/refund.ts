...
export interface RefundRequest {
  ...
  paymentMethod: 'payper' | 'usdc'; // Hvilken token der skal refunderes
...
/**
 * Sender PAYPER token refund til bruger
 */
async function sendPAYPERRefund(
  connection: Connection,
  refundWalletKeypair: Keypair,
  userPublicKey: PublicKey,
  usdAmount: number
  ...
  try {
    console.log('💰 Sending PAYPER refund...');

    // Get current PAYPER token price
    const tokenPrice = await getTokenPriceUSD();
    console.log('💵 Current $PAYPER price:', `$${tokenPrice}`);

    // Calculate how many PAYPER tokens to send
    const tokenAmount = usdAmount / tokenPrice;
    const tokenAmountRaw = Math.floor(tokenAmount * Math.pow(10, TOKEN_DECIMALS));

    console.log('🔢 PAYPER amount:', tokenAmount.toFixed(2), '$PAYPER');
    ...
    if (!fromAccountInfo) {
      return {
        success: false,
        error: 'Refund wallet does not have a PAYPER account',
      };
    }
    ...
    console.log('💰 Refund wallet PAYPER balance:', currentBalance.toFixed(2), '$PAYPER');

    if (currentBalance < tokenAmount) {
      return {
        success: false,
        error: `Insufficient PAYPER in refund wallet! Has ${currentBalance.toFixed(2)} $PAYPER, needs ${tokenAmount.toFixed(2)} $PAYPER`,
      };
    }
    ...
    if (!toAccountInfo) {
      return {
        success: false,
        error: 'User does not have a PAYPER account to receive refund',
      };
    }
    ...
    console.log('📤 Sending PAYPER refund transaction...');
    ...
    console.log('✅ PAYPER refund successful!');
    console.log('🔗 Transaction:', `https://solscan.io/tx/${signature}`);
    ...
    console.error('❌ PAYPER refund error:', error);
    return {
      success: false,
      error: error.message || 'PAYPER refund failed',
    };
  }
}

/**
 * Henter PAYPER token pris fra DexScreener
 */
